...
touch /usr/local/etc/suricata/entrante-no-solicitado.rules 2> /dev/null
echo 'alert icmp $EXTERNAL_NET any -> $HOME_NET any (msg: "Detectada conexión ICMP entrante no solicitada"; flow:not_established; sid:5000001; rev:1;)' >> /usr/local/etc/suricata/entrante-no-solicitado.rules
